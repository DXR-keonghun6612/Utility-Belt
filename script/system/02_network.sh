...
list_network_interfaces() {
    ...
    # Bash의 Globbing을 사용하여 ls | grep 방식(SC2010)을 피하고 안전하게 목록을 가져옵니다.
    local interfaces=()
    for dev_path in /sys/class/net/*; do
        [[ -e "$dev_path" ]] || continue
        local dev_name="${dev_path##*/}"
        
        # lo(루프백), bond, br- 등 가상 인터페이스를 제외.
        if [[ ! "${dev_name}" =~ ^(lo|bond|br-|veth|docker) ]]; then
            interfaces+=("${dev_name}")
        fi
    done
...
##
# @description 특정 인터페이스의 현재 IP, 게이트웨이, DNS 설정을 가져옴.
...
#
get_current_ip_info() {
    log_info "Fetching current IP address information..."
...
check_connection_status() {
    ...
    log_info "Pinging ${target_host} to check network connectivity..."
    
    if ping -c 4 "${target_host}"; then
        log_success "Connection to ${target_host} is successful."
        return 0
    else
        log_error "Connection to ${target_host} failed."
        return 1
    fi
}
...
set_host_ip_mapping() {
    ...
    if [[ -z "${ip_address}" || -z "${hostname}" ]]; then
        log_error "IP address and hostname are required."
        return 1
    fi
    ...
    log_info "Preparing update for ${hosts_file}: ${hostname} -> ${ip_address}..."
    ...
    if (( commit_status == 0 )); then
        log_success "${hosts_file} has been updated successfully."
        return 0
    else
        log_error "Failed to apply changes to ${hosts_file}."
        return 1
    fi
}
...
set_hostname() {
    ...
    if [[ -z "${new_hostname}" ]]; then
        log_error "New hostname cannot be empty."
        return 1
    fi

    if ! command -v hostnamectl &> /dev/null; then
        log_error "'hostnamectl' is not available. This function requires a systemd-based system."
        return 1
    fi

    log_info "Changing hostname to '${new_hostname}'..."
    if ${G_SUDO_PREFIX} hostnamectl set-hostname "${new_hostname}"; then
        log_success "Hostname successfully changed to '${new_hostname}'."
        log_info "Note: A reboot or new login session may be required for the change to be fully reflected."
    else
...
_set_static_ip_nmcli() {
    ...
    if ${G_SUDO_PREFIX} nmcli con mod "${interface}" ipv4.method manual ipv4.addresses "${ip_cidr}" ipv4.gateway "${gateway}" ipv4.dns "${dns}" && \
       ${G_SUDO_PREFIX} nmcli con down "${interface}" && ${G_SUDO_PREFIX} nmcli con up "${interface}"; then
        ...
        log_error "Failed to apply IP settings using nmcli."
        return 1
    fi
}
...
_set_static_ip_networkd() {
    ...
    log_info "Creating network configuration for systemd-networkd..."
    
    local network_config="[Match]\nName=${interface}\n\n[Network]\nAddress=${ip_cidr}\nGateway=${gateway}\nDNS=${dns}"
    
    # 설정 파일을 생성
    if ! echo -e "${network_config}" | ${G_SUDO_PREFIX} tee "${conf_file}" > /dev/null; then
        log_error "Failed to write network configuration to ${conf_file}."
        return 1
    fi

    log_info "Applying configuration for '${interface}' without a full service restart..."
    if ${G_SUDO_PREFIX} networkctl reconfigure "${interface}"; then
        log_success "Successfully applied settings for '${interface}' via systemd-networkd."
        # systemd-resolved를 사용하는 경우 재시작하여 DNS 변경 사항 즉시 적용
        if systemctl is-active --quiet systemd-resolved; then
            ${G_SUDO_PREFIX} systemctl restart systemd-resolved
            log_info "Restarted systemd-resolved to apply new DNS settings."
        fi
        return 0
    else
        log_error "Failed to reconfigure interface '${interface}' using networkctl."
        return 1
    fi
}

##
# @description 지정된 인터페이스에 고정 IP를 설정.
#
set_static_ip() {
...
create_network_bond() {
    ...
    if nmcli con show "${bond_name}" &> /dev/null; then
        log_error "Bond '${bond_name}' already exists."
        return 1
    fi

    log_info "Creating network bond '${bond_name}'..."
    
    # 1. 본드 마스터 연결 프로필 생성
    if ! ${G_SUDO_PREFIX} nmcli con add type bond con-name "${bond_name}" ifname "${bond_name}" bond.options "mode=${mode}"; then
        log_error "Failed to create bond master."; return 1
    fi

    # 2. 쉼표로 구분된 문자열을 배열로 변환해 각 슬레이브를 추가
    IFS=',' read -ra slaves <<< "${slaves_str}"
    for slave in "${slaves[@]}"; do
        log_info "Adding slave interface '${slave}' to '${bond_name}'..."
        ...
        if ! ${G_SUDO_PREFIX} nmcli con add type bond-slave con-name "${bond_name}-slave-${slave}" ifname "${slave}" master "${bond_name}"; then
            log_error "Failed to add slave '${slave}'."; return 1
        fi
    done
    
    # 3. 본드 활성화
    if ${G_SUDO_PREFIX} nmcli con up "${bond_name}"; then
        log_success "Network bond '${bond_name}' created and activated successfully."
    else
