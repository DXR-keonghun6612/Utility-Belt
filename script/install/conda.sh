...
# -----------------------------------------------------------------------------
# @description Conda (Anaconda/Miniconda) 설치 여부 확인 및 설정 동기화
...
is_installed_conda() {
    ...
    if [[ -n "${G_STATE_FILE}" ]]; then
        if [[ $installed -eq 0 ]]; then
            local current_val
            current_val=$(get_config_value "${G_STATE_FILE}" "APPLICATION_LIST" "conda")
            if [[ -z "${current_val}" ]]; then
                local timestamp; timestamp=$(date "+%Y-%m-%dT%H:%M:%S")
                add_config_section "${G_STATE_FILE}" "APPLICATION_LIST"
                set_config_value "${G_STATE_FILE}" "APPLICATION_LIST" "conda" "${timestamp}"
            fi
        else
            delete_config_value "${G_STATE_FILE}" "APPLICATION_LIST" "conda"
        fi
    fi
...
install_conda_logic() {
    ...
    if [[ "${conda_type}" == "anaconda" ]]; then
        log_info "Detecting latest Anaconda version..."
        installer_file=$(_get_latest_anaconda_filename)
        
        if [[ -z "${installer_file}" ]]; then
            log_warn "Failed to detect latest Anaconda version. Using fallback version."
            installer_file="Anaconda3-2025.12-1-Linux-x86_64.sh"
        else
            log_info "Latest Anaconda version detected: ${installer_file}"
        fi
    ...
    if [[ -d "$install_prefix" ]]; then
        log_warn "${conda_type} appears to be already installed at $install_prefix."
        return 0
    fi

    # --- 3. 다운로드 ---
    local installer_tmp_path="/tmp/${installer_file}"
    log_info "Downloading ${conda_type} installer..."
    if ! wget "${installer_url}" -O "${installer_tmp_path}"; then
        log_error "Failed to download ${conda_type} installer."
        return 1
    fi
    
    # --- 4. 설치 실행 (Silent Mode) ---
    log_info "Installing ${conda_type} to $install_prefix (Mode: $install_mode)..."
    ...
    if [[ "$install_mode" == "system" ]]; then
        ...
        log_info "System-wide configuration added to /etc/profile.d/conda.sh"
    else
        # 사용자 설정: ~/.bashrc 수정
        if [[ -x "$conda_bin" ]]; then
            "$conda_bin" init bash
    ...
    log_success "${conda_type} installed successfully."
    return 0
}