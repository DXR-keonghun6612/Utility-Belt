...
def per_pixel_stats(
    ...
    ``Robust_mean_std`` 가 leading shape 무관하게 ``(H,W,B)`` 를 한 번에 ``(H,W)`` 로 처리한다.
    ...
    if row_chunk is None:
        _mean, _std = Robust_mean_std(hist, circular)
    ...
    for _y0 in range(0, _H, row_chunk):
        ...
        _m, _sd = Robust_mean_std(hist[_y0:_y1], circular)
        _mean[_y0:_y1] = _m
        _std[_y0:_y1]  = _sd
    return _mean, _std, _n
...
def global_stats(hist: np.ndarray, window: int, circular: bool) -> tuple[float, float]:
    ...
    _m, _sd = Robust_mean_std(_g, circular)
...
def global_robust_from_pixels(
    ...
    **한 표**일 뿐이라 공간 다수(벨트)에 묻힌다. robustness 를 시간축→공간축으로 옮기는 것.
    ...
    _m, _sd = Robust_mean_std(_votes, circular)
    return float(_m), float(_sd)
