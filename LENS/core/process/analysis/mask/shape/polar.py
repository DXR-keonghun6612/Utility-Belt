...
설계 의도(네 아이디어): 원점 기준 각 (h,w) 픽셀의 (r, θ-bin) 을 **사전계산한 LUT** 를 두고, mask
는 centroid 가 캔버스 중심에 오도록 정수 시프트만 한 뒤 LUT 로 매핑한다. 매 mask 마다 hypot/atan2
를 다시 돌리지 않으니(같은 크기 캔버스면 LUT 재사용) 직관적이고 빠르다. resolution(θ bin 수)이
유일한 핵심 인자.

한 θ 에 경계가 여러 번 걸리는 경우(도넛형·centroid 가 빈 공간) 를 위해 **최외각 r_outer 와
최내각 r_inner 두 프로파일**을 낸다:
  - 솔리드·star-convex: r_inner ≈ 0 (사실상 한 겹)
  - 도넛/centroid 가 구멍 안: r_inner = 구멍 반경, r_outer = 바깥 반경
외각만으로 대부분 구분되고, 내각을 더하면 특이 케이스 빼고 거의 구분된다. 그 이상은 필요할 때.
"""
...
def mask_to_polar(
    mask: np.ndarray, resolution: int, fill_gaps: bool = True
