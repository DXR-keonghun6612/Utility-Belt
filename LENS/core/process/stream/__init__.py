...
각 도메인(preprocess/mask/edge/chroma/model/select)의 순수 도메인 함수. 엔진 계약(``Base_Process``·
표시 힌트 ``UI``·타입 별칭 ``BBOX``/``GRAY_IMAGE``)과 레지스트리(``PROCESS_REGISTRY``)를 여기서
...
__all__ = ["PROCESS_REGISTRY", "Base_Process", "UI", "BBOX", "GRAY_IMAGE"]
