...
``fill.color_holes`` 와 **같은 메커니즘의 대칭**(LoG 벽 + ``cv2.floodFill``)이되 방향이 반대다 — 저쪽은
*있는 mask 안*에서 몸통색과 다른 구멍을 파내고, 여기서는 *맨 프레임*의 배경을 채운 뒤 반전해 객체를
얻는다. Canny 윤곽을 이어 닫아 채우는 경로와 달리 **edge 를 닫을 필요가 없다** — 배경 쪽에서 흐르다
막히는 곳이 곧 객체 경계이기 때문이다.
"""
