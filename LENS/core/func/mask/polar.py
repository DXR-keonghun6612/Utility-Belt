...
def Hollowness(mask: GRAY_IMAGE, resolution: int = 256) -> float | None:
    """중심 구멍 비율 ``mean(r_inner) / mean(r_outer)`` ∈ ``[0, 1]``. 전경이 없으면 None.

    ``0`` = 꽉 참(내곽 반경 ≈ 0), ``1`` 에 가까울수록 얇은 링(중심 구멍이 큼). 외곽 실루엣이 같아도
    중심 구멍의 유무를 이 스칼라 하나로 구분한다 — 두께 ``t = r_outer - r_inner`` 를 반경으로 정규화한
    것과 같다.
    ...
    Returns:
        hollowness. 전경이 없거나 외곽 반경 평균이 ``0`` 이하면 None.
    """
    if not np.any(mask):
        return None
    _r_out, _r_in = mask_to_polar(np.asarray(mask), resolution)
    _mean_out = float(np.mean(_r_out))
    if _mean_out <= 0:
        return None
    return float(np.clip(np.mean(_r_in) / _mean_out, 0.0, 1.0))
