...
임의 깊이로 중첩되고, 사이드카 영속(``store_io.Scatter``/``Restore``)이 그 중첩을 재귀 직렬화로 담는다. 즉
...
class Sample_Set(Bucket_Store):
    ...
    은 그 의미를 모르고 range(split)만 고정한다. 영속·전이·병합·번들(Gather)은 ``store_io`` 자유함수.
    """
