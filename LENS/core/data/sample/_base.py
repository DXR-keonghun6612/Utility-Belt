...
사이드카 영속(``store_io.Scatter``/``Restore``)이 그 중첩을 재귀 직렬화로 담는다. 즉 ``Sample_Set`` 은
...
class Sample_Set(Bucket_Store):
    """파생 학습셋 store — 범주 = 단일 작업 버킷(``WORKING``). split 은 담지 않는다.

    분석·재배정은 split 을 몰라도 되므로(class 이동뿐) 작업 store 는 **split 없는 단일 버킷**만 두고,
    그 안의 트리 모양은 task(sink)가 정한다 — classification 은 class(그 안에 sample 중첩), detection 은
    image(그 안에 object 중첩). train/val/test 는 파생(frame stem 해시)이라 **내보내기 때만** 갈라
    ``{split}/…`` 로 실체화한다(``core/sampler/export``). ``Sample_Set`` 자체는 split 을 모른다.
    영속·전이·병합·번들(Gather)은 ``store_io`` 자유함수.
    """
