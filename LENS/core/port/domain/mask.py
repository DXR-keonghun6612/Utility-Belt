...
**라벨맵(``segmap``)은 지금 [`raster_domains`](raster_domains.py) 의 형제 도메인이다** — 하지만 여러 mask 를
한 장에 겹쳐 담은 저장 표현일 뿐이라 **mask 의 한 포맷으로 접는 것**이 방향이다(카디널리티는 도메인을
가르지 않는다 — 라벨맵에서 mask 하나 꺼내기 ``Mask_of`` 는 별도 도메인이 아니라 인자 있는 변환일 뿐).
아직 코드는 분리돼 있다 → [`../../TODO.md`](../../TODO.md) "★ 축 정리".
