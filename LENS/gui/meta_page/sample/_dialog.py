...
레시피로 영속되며, 목록은 ``Pipeline.Taskers()`` 가 소스. Processes 체인(공통 ``gui.steps.Step_list``)이
...
class Sampler_dialog(Pop_dialog):
    ...
    def _build(self) -> None:
        ...
        # 체인이 곧 실체화(materialize) — Step_list(min_count=1)로 최소 1 step 강제. 비면
        # payload 없는 껍데기 sample 이 되므로 허용하지 않는다. 최종 출력 key ``crop`` 이
        # sample payload 로 저장된다(step 의 slots 로 재배선 가능).
