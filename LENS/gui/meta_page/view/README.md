...
- 좌 `Stem_list` — **3-버킷 상태 뱃지(작업/검수/보류)**. 우클릭 = 현재 상태가 아닌 **각 상태로 보내기**·삭제,
  ...
  순서(`STATES`)는 유지하되 카테고리 안은 stem 이름 오름차순**으로 정렬한다(전이로 순서가 안 뒤섞이게).
...
(`transition_requested`/`remove_requested`), 상위(app `Meta_ops`)가 워커로 `store_io.Move`/`store_io.Delete` 를 돌리며
...
`Idmap_panel`·열린 팝아웃으로 전파하고, 작업 중 새로 뜬 편집기/팝아웃에도 현재 잠금을 적용한다.
...
데이터 라이프사이클은 store 가 소유한다 — 상태 전이 = `store_io.Move(meta, …)`, 삭제 = `store_io.Delete(meta, …)`,
편집 저장 = `store_io.Save_item(meta, stem)` (호출 측이 `store_io.*` 직접). meta 정체성이 바뀌면(`set_pipeline`) 편집기를 폐기·재바인딩한다.
