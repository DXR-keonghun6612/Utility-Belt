...
- **`Sample_viewer`**(비모달, tasker당) — `Pipeline.Load_sample(name)` 트리. sample 선택 시 **crop payload
  (있으면)** 또는 **정본 프레임 역참조**(`source_stem`→meta, `gui/verify/_overlay` 재사용)로 미리보기.
...
1. `source_stem`/`source_obj` 역참조로 정본 obj 를 찾아 `Set_attr(obj, "class_id", new)` + `meta.Save_item`.
...
geometry(mask/segment)는 여기서 안 건드린다(`gui/verify` 의 `Stem_editor` 소유). "학습셋에서 빼기"(exclude)
