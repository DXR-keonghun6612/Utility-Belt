...
class Image_label(QWidget):
    ...
    Attributes:
        zoom_changed: 유효 배율을 emit하는 시그널. ``0.0`` 은 "fit" 모드를 뜻한다.
    ...
    Note:
        ...
        기준 절대 배율로 고정된다. fit 모드에서는 창 리사이즈 시 자동으로 다시 맞춘다.
        ``set_interactive(True)`` 면 라벨 위 마우스 좌표를 원본 픽셀로 환산해 시그널로 낸다
        (줌/스크롤 무관) — bbox 그리기/편집 같은 상위 인터랙션을 위함.
    """

    zoom_changed   = Signal(float)  # 유효 배율 emit; 0.0 은 "fit" 모드 신호
