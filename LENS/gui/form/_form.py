...
class Config_form(QWidget):
    ...
    def load(self, params: dict) -> None:
        """저장된 값으로 폼을 복원한다 (시그널 차단; 폼에 없는 키는 무시)."""
        for _name, _val in params.items():
            ...
            if isinstance(_w, tuple):
                ...
                if _val is not None:
                    _slider._spin.blockSignals(True)
                    _slider._slider.blockSignals(True)
                    _slider._spin.setValue(float(_val))
                    _slider._spin.blockSignals(False)
                    _slider._slider.blockSignals(False)
            elif isinstance(_w, QCheckBox):
                ...
                _w._spin.blockSignals(True)
                _w._slider.blockSignals(True)
                _w._spin.setValue(_val)
                _w._spin.blockSignals(False)
                _w._slider.blockSignals(False)
