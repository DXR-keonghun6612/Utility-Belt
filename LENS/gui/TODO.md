...
## ❓ 논의 대상 — 객체 `pose` 의 신뢰도를 함께 적을까

저장 정돈이 객체마다 `pose`(자리 + quaternion) 하나를 적는다(`_normalize.Fit_pose`). 그 회전은
flow 의 `radial_profile` 이 feature 파이프라인에서 꺼내 온 주축각인데, **그 사슬은 각과 함께
신뢰도 둘을 낸다**:

- `anisotropy` — 주축이 실재하는가. 원환·n≥3 회전대칭(플랜지·기어)에서 **정확히 0** 이고, 그때 각은
  노이즈가 정한 값이다.
- `flip_margin` — 180° 확정이 견고한가. 2회 대칭에서 정확히 0.

지금은 자세 하나만 남기므로 **읽는 쪽이 "이 각이 뜻이 있나"를 알 길이 없다.** 셋 중 하나:

- 함께 적는다 (`pose_trust: [aniso, flip]`) — 정직하지만 객체마다 필드가 둘이 된다.
- 임계 아래면 `pose` 를 안 적는다 — 임계가 곧 정책이라 여기서 정할 수 없다.
- 그대로 둔다 — 소비처가 생길 때 그쪽이 mask 에서 다시 재게 한다.

**소비처가 정해지면 답이 나온다** (로봇 픽업이면 신뢰도가 필수, 통계 요약이면 불필요).
