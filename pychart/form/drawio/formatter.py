...
def F_attributes(
    ...
    for _a in attributes:
        _a_name = replace_brackets(_a.name)
        _a_type = replace_brackets(_a.type_hint)
...
def F_methods(
    ...
    _data = []
    for _m in methods:
        _m_name = replace_brackets(_m.name)
        ...
        _m_rt_type = replace_brackets(_m.return_type)
        ...
        if not _valid_args:
            ...
            for _arg in _valid_args:
                _t = replace_brackets(_arg.type_hint)
                _val += f"&nbsp;&nbsp;&nbsp;&nbsp;{_arg.name}: {_t},<br>"
            _val += f") -&gt; {_m_rt_type}"
            _h = default_h + (len(_valid_args) * default_h) + 16
            
        _data.append((replace_brackets(_val), _h, "function"))
    return _data