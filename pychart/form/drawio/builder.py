"""builder.py: Draw.io 그래프 빌더 엔진 (모듈 캡슐화 + 사용자 계층 + 정밀 포트 레이아웃)."""
from pathlib import Path
from typing import Any
from pychart.definition import Arg_Info, Method_Info, Class_Info, Module_Info
...
class Graph_Builder:
    ...
    def __init__(self) -> None:
        ...
        # 정밀 포트 연결을 위한 맵 (symbol_id -> mx_cell_id)
    ...
    def Build_from_graph(self, graph_data: Any, is_detailed: bool = False, current_pkg: str = "") -> str:
        ...
        for _id, _obj in graph_data.nodes.items():
            if _id.startswith("stub:"):
                ...
                if _mod_path not in _module_contents: _module_contents[_mod_path] = []
                _module_contents[_mod_path].append(_id)

        # 2. 글로벌 랭킹 계산 (모듈 상자들의 수직 위치 결정)
        ...
        for _m, _r in _module_ranks.items():
            if _r not in _rank_to_items: _rank_to_items[_r] = []
        ...
        for _r in sorted(_rank_to_items.keys()):
            ...
            for _item in sorted(_rank_to_items[_r]):
                ...
                if _type == "mod":
                    # 모듈 상자 생성
                    _c_id, _w, _h = self._Render_module_container(_val, _module_contents[_val], graph_data, _current_x, _current_y, is_detailed)
                    _current_x += _w + 150
                    _max_h_in_rank = max(_max_h_in_rank, _h)
                else:
                    # 외부 스터브 생성
                    _p_id, _h = self._Render_single_node(_val, graph_data.nodes[_val], _current_x, _current_y, is_detailed)
        ...
        for _edge in graph_data.edges:
            ...
            if _src_cell and _tgt_cell:
                ...
                self._Render_precision_edge(_src_cell, _tgt_cell, _edge.edge_type, _tgt_obj, _edge.target_id)
                
        return self._Generate_xml()

    def _Calculate_user_logic_ranks(self, members: list[str], edges: list) -> dict[str, int]:
        """사용자 정의 참조 레벨 알고리즘."""
        ...
        for _ in range(10):
            _changed = False
            for m in members:
                # m이 '의존하는' 대상들(부품들) 찾기 (Edge: Target -> Source 구조)
                # target_id가 부품이고 source_id가 조립체임
                _parts = [e.source_id for e in edges if e.target_id == m and e.source_id in _member_set]
                _new_rank = 0
                if not _parts: _new_rank = 0
                elif len(_parts) == 1: _new_rank = _ranks.get(_parts[0], 0)
                else: _new_rank = max(_ranks.get(p, 0) for p in _parts) + 1
    ...
    def _Render_module_container(self, mod_path: str, member_ids: list, graph_data: Any, x: int, y: int, is_detailed: bool) -> tuple[str, int, int]:
        ...
        for m in member_ids:
            r = _local_ranks[m]
            if r not in _rank_groups: _rank_groups[r] = []
        ...
        for _r in sorted(_rank_groups.keys()):
            ...
            for _mid in sorted(_rank_groups[_r]):
                ...
                _child_id, _actual_h = self._Render_single_node(_mid, _m_obj, _inner_x, _inner_y, is_detailed, parent=_container_id)
                _inner_y += _actual_h + MARGIN_Y
            _max_total_h = max(_max_total_h, _inner_y)
            _inner_x += DEFAULT_WIDTH + 150
            
        _style = get_swimlane_style(MODULE_HEADER_HEIGHT, THEME_MODULE["header"], THEME_MODULE["stroke"], use_stack=False)
        self.cells.append(Mx_Cell(
            id=_container_id, value=f"«module»#{mod_path}", style=_style, vertex="1", parent="1",
    ...
    def _Render_single_node(self, node_id: str, obj: Any, x: int, y: int, is_detailed: bool, parent: str = "1") -> tuple[str, int]:
        _stereotype = getattr(obj, "stereotype", "")
        if isinstance(obj, Class_Info):
            _children, _theme = F_attributes(obj.attributes) + F_methods(obj.methods, is_detailed=is_detailed), THEME_CLASS
        elif isinstance(obj, Method_Info):
            _children, _theme = F_methods([obj], is_detailed=is_detailed), THEME_METHOD
        elif isinstance(obj, Module_Info):
            _children, _theme = [], THEME_MODULE
        else: return self._Next_id(), 0
        
        _link = f"#{node_id}"
        if node_id.startswith("stub:"):
            _fp = getattr(obj, "file_path", None)
            _link = f"#{Path(_fp).as_uri()}" if _fp else f"#{node_id.split(':')[1].rpartition('.')[0]}.drawio"

        _parent_id = self._Create_swimlane_with_ports(node_id, obj.name, _stereotype, _children, x, y, _theme, link=_link, parent=parent)
    ...
    def _Create_swimlane_with_ports(self, symbol_id: str, name: str, stereotype: str, children_data: list, x: int, y: int, theme_colors: dict, link: str = None, parent: str = "1") -> str:
        ...
        self.cells.append(Mx_Cell(
            id=_parent_id, value=f"{stereotype}#{name}", style=get_swimlane_style(_start_size, theme_colors["header"], theme_colors["stroke"], use_stack=True),
            vertex="1", parent=parent, geometry=Mx_Geometry(x=x, y=y, width=DEFAULT_WIDTH, height=_total_h), link=link
        ))
        ...
        for _val, _h, _type in children_data:
            ...
            self.cells.append(Mx_Cell(id=_child_id, value=_val, style=get_child_style(_bg), vertex="1", parent=_parent_id, geometry=Mx_Geometry(y=_cur_y, width=DEFAULT_WIDTH, height=_h)))
            _cur_y += _h
        return _parent_id

    def _Render_precision_edge(self, source_cell: str, target_cell: str, edge_type: str, target_obj: Any, target_id: str) -> None:
        ...
        _style.update({"perimeter": "0", "exitX": "1", "exitY": "0.5", "entryX": "0", "entryY": f"{_entry_y:.2f}"})
        self.cells.append(Mx_Cell(
            id=self._Next_id(), value="", style=_style, vertex=None, edge="1", parent="1",
            source=source_cell, target=target_cell, geometry=Mx_Geometry(x=0, y=0, width=0, height=0, relative="1", as_attr="geometry")
        ))
    ...
    def _Generate_xml(self) -> str:
        ...
        for cell in self.cells:
            ...
            _attrs = " ".join(f'{k}="{Escape_str_for_xml(str(v))}"' for k, v in _d.items() if v is not None)
            if _geo:
                _g_attrs = " ".join(f'{k}="{Escape_str_for_xml(str(v))}"' for k, v in _geo.items() if v is not None)
                _xml.append(f"<mxCell {_attrs}><mxGeometry {_g_attrs}/></mxCell>")
            else: _xml.append(f"<mxCell {_attrs}/>")
